...
with col2:
    show_details = st.toggle("הצג נתונים מפורטים בהשוואה", value=False, key="show_detailed_data")
...
if st.session_state.Do_Calc_Indicator == True:
    ...
    columns = pd.MultiIndex.from_tuples([
        ('', 'Year'),
        (deal0_name, 'Deposit Returned'),
        (deal0_name, 'PV Deposit Returned'),
        (deal0_name, 'PV Rent'),
        (deal0_name, 'PV Total'),
        (deal0_name, 'PV Monthly Avarage'),
        (deal1_name, 'Deposit Returned'),
        (deal1_name, 'PV Deposit Returned'),
        (deal1_name, 'PV Rent'),
        (deal1_name, 'PV Total'),
        (deal1_name, 'PV Monthly Avarage'),
    ])
    ...
    # column_names ={
    #     'Year': 'שנה',
    #     'Deposit Returned': 'פיקדון מוחזר',
    #     "PV Deposit Returned": 'ערך נוכחי של פיקדון מוחזר',
    #     "PV Rent": 'ערך נוכחי של תשלומים חודשיים',
    #     "PV Total": 'סך עלות דיור בערך נוכחי',
    #     "PV Monthly Avarage": 'ממוצע עלות לחודש בערך נוכחי'
    #     }
    # df = pd.DataFrame(off0_results)
    ...
    # if show_details:
    #     columns_to_show = ['שנה', 'פיקדון מוחזר', 'ערך נוכחי של פיקדון מוחזר',
    #                        'ערך נוכחי של תשלומים חודשיים', 'סך עלות דיור בערך נוכחי',
    #                        'ממוצע עלות לחודש בערך נוכחי']
    # else:
    #     columns_to_show = ['שנה', 'סך עלות דיור בערך נוכחי', 'ממוצע עלות לחודש בערך נוכחי']
    
    # # Display as RTL table
    # html = df_formatted[columns_to_show].to_html(index=False)
    # rtl_html = f"""
    # <div dir="rtl" style="text-align: right">
    # {html}
    # </div>
